...
- MCP server 暴露频道工具：`reply`、`edit_message`、`broadcast`、`send_file`、`set_user_name`、`share_doc`
- `share_doc` 把长 markdown（plan / spec / 审阅文档）发布成 cloudflared quick tunnel 的公网 URL，微信用户点开就能在手机浏览器里看到渲染好的内容 —— 无需 GitHub 账号、无需服务器、零配置
...
- `cloudflared` —— `share_doc` 工具用它把本地渲染的 markdown 页面透过
  quick tunnel 暴露到 `*.trycloudflare.com`。**你不需要自己装** —— 首次
  调用 share_doc 时 wechat-cc 会自动下载对应架构的静态二进制到
...
微信文本消息无法渲染 markdown —— 代码块、表格、嵌套列表都会糊成一团，在手机上没法读。`share_doc` 这个 MCP tool 用来解决这个：把一篇 markdown 发布成一个短期 URL，微信用户点开直接在手机浏览器里看到渲染好的页面。

**工作流程：**

1. Claude 调用 `share_doc({ title, content, chat_id? })`
2. 正文写到 `~/.claude/channels/wechat/docs/<slug>.md`
3. wechat-cc 在本地起一个 `Bun.serve`（随机端口），通过 `marked` 把 `/docs/<slug>` 渲染成带简洁样式的 HTML
...
5. `share_doc` 返回 `https://<tunnel>.trycloudflare.com/docs/<slug>`；如果带了 `chat_id`，顺手发一条微信消息（标题 + 预览 + URL），一次工具调用覆盖"发布 + 通知"

**Claude 什么时候用它：** plan、spec、设计提案、长篇审阅文档，或者任何多段、带结构（代码、表格、列表）的内容。MCP server instructions 里已经写清楚了判断条件。

**注意事项：**

- URL 是公开可访问的，任何拿到 URL 的人都能读。不要在 share_doc 里放**密钥、凭据、内部战略**这种东西。slug 是 4 个随机单词 + 时间戳后缀，不容易被暴力猜到，但不是鉴权手段
- URL 是临时的：`wechat-cc run` 退出后 cloudflared 进程也会被一起带走，旧 URL 立刻失效。本地的 `.md` 文件会保留，Claude 下次启动可以重新 share 拿一个新 URL
- Cloudflare 官方把 quick tunnel 定位为非生产用途，个人/小团队用没事，大流量场景不合适
- 内容会经过 Cloudflare 边缘节点。如果你介意这点，要么只用 share_doc 发不敏感内容（这是设计初衷），要么从 server.ts 里把这个 tool 去掉
...
├── docs/                  # share_doc 发布的 markdown 正文（<slug>.md）
├── bin/
│   └── cloudflared        # 首次 share_doc 时自动下载
├── inbox/                 # 下载的媒体文件
└── accounts/
    └── <bot_id>/
        ├── account.json
