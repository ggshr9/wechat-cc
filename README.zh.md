...
> ⚠️ 安装前必读 [账号封禁风险](#账号封禁风险)。ilink 是合作伙伴专用的微信接口；本项目把它挪用了。账号有被限制的可能。
...
ilink 是微信给 OpenClaw 等合作平台用的接口。**用它对接 Claude Code 不在它的设计意图内**。可能后果：

- 微信可能随时撤销 / 限制 bot 的会话（你会在 `/health` 看到 `errcode=-14`）
- 重度使用、账号共享、明显自动化模式可能让微信账号本体被限制
- 未来一次 ilink 更新就可能完全堵死这条路

如果上面任何一种你都接受不了，别装。这条路上没保险——你的账号，你来决定。
...
ilink 是微信提供的合作伙伴通信接口，用于对接 OpenClaw 等平台。wechat-cc 把它用于 Claude Code 集成，**不在其设计意图内**。这种用法可能不被微信允许，相关账号可能被限制。
