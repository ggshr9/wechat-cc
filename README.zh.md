...
**Windows 用户**：wechat-cc 支持 Windows（Bun 1.1+ 原生支持）。`share_page` 的
cloudflared 自动下载和所有 MCP 工具都能跨平台运行。唯一的退化是 `/restart`
自动确认开发通道对话框 —— Windows 上没有 `expect(1)`，重启时需要手动在终端按
一次回车。其余功能和 Linux/macOS 完全一致。
...
├── channel.log            # 滚动日志（超 10MB 自动 rotate 成 .1）
├── server.pid             # 单实例锁
...
├── bin/
│   └── cloudflared        # 首次 share_page 时自动下载（Windows 上是 .exe）
├── inbox/                 # 下载的媒体文件（30 天 TTL，启动时自动清理）
└── accounts/
    └── <bot_id>/
        ├── account.json
